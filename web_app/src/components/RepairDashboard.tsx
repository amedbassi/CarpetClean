...
export default function RepairDashboard() {
    ...
    const sendRepairApprovalRequest = async (order: RepairOrder) => {
        ...
        if (!hasRepairItems) {
            alert(t.repair.no_estimates_found);
            return;
        }
        ...
        try {
            ...
            alert(t.repair.estimate_sent_success(order.client.email));
            window.location.reload();
        } catch (error) {
            console.error('Error sending repair estimate:', error);
            alert(t.repair.failed_send_repair(error instanceof Error ? error.message : t.common.error, order.client.email));
            window.location.reload();
        }
    };
            ...
            <div className="space-y-4">
                                    ...
                                    <button
                                        ...
                                        title={
                                            !order.repairItems.some(item => (item.repairCost || 0) > 0)
                                                ...
                                                : (order.repairApprovalStatus === 'pending' || order.repairApprovalStatus === 'approved')
                                                    ? t.repair.estimate_already_sent
                                    ...
                                    <div className="flex flex-col gap-3">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <span className={`text-xs font-bold ${item.state === 'Damaged' ? 'text-red-500' : 'text-orange-500'}`}>
                                                    {item.state ? (t.repair[item.state.toLowerCase() as 'worn' | 'damaged'] || item.state) : ''}
                                                </span>
                                            </div>
                ...
                {repairOrders.length === 0 && (
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center text-gray-400">
                        ...
                        <p className="font-medium">{t.repair.no_rugs_repair}</p>
                    </div>
                )}
            </div>
